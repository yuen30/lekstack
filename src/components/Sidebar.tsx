...
import { LayoutDashboard, Download, Settings, Globe, FileText, Database } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ activeView, onNavigate }) => {
  ...
  return (
    <aside className="w-16 bg-white dark:bg-[#151515] border-r border-gray-100 dark:border-gray-800 flex flex-col items-center h-screen transition-all select-none py-4">
      {/* Logo */}
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center text-white font-bold text-xl mb-6">
        L
      </div>
      ...
      <nav className="flex-1 flex flex-col items-center w-full gap-1">
        {menuItems.map((item) => (
          ...
          >
            ...
            {/* Tooltip */}
            <span className="absolute left-[80%] ml-4 px-2 py-1 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 text-xs rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
              {item.label}
            </span>
          </button>
        ))}
      ...
      <div className="relative group">
        ...
        <span className="absolute left-full ml-4 px-2 py-1 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 text-xs rounded-lg opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
          All systems normal
        </span>
      </div>
    </aside>
